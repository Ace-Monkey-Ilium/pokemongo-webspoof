...
const updateXcodeLocation = throttle(([ lat, lng ]) => {
  ...
  const latJitter = settings.addJitterToMoves.get() ? random(-0.000009, 0.000009, true) : 0
  const lonJitter = settings.addJitterToMoves.get() ? random(-0.000009, 0.000009, true) : 0

  const xcodeLocationData =
    `<gpx creator="Xcode" version="1.1"><wpt lat="${(lat + latJitter).toFixed(6)}" lon="${(lng + lonJitter).toFixed(6)}"><name>PokemonLocation</name></wpt></gpx>`
